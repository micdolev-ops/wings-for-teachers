...
const Header = () => {
  ...
  const navItems = [
    { label: "בית", href: "/" }, { label: "Canva להוראה", href: "/canva" },
    { label: "בינה מלאכותית", href: "/ai" }, { label: "אודות", href: "/about" }, { label: "צור קשר", href: "/contact" },
    { label: "למידה לאורך החיים", href: "/lifelong" },
  ];
  return (
      ...
      <div className="container mx-auto px-4 md:px-6">
        <div className="flex h-24 md:h-32 items-center justify-between">
          ...
          <nav className="hidden md:flex items-center gap-1">
            {navItems.map((item) => (
              <Link key={item.href} to={item.href} className={cn("px-4 py-2 rounded-lg text-sm font-medium transition-all duration-200",
          ...
          <nav className="md:hidden py-4 border-t border-border/40">
            {navItems.map((item) => (
              <Link key={item.href} to={item.href} onClick={() => setIsMenuOpen(false)}
                className={cn("block px-4 py-3 rounded-lg text-base font-medium transition-all duration-200",
                  location.pathname === item.href ? "bg-primary/10 text-primary" : "text-muted-foreground hover:text-foreground hover:bg-muted")}>
                {item.label}
              </Link>
            ))}
